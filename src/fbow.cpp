...
void Vocabulary::setParams(int aligment, int k, int desc_type, int desc_size,
                           int nblocks, std::string desc_name) {
    auto ns =
        desc_name.size() < static_cast<size_t>(49) ? desc_name.size() : 128;
    desc_name.resize(ns);
    ...
    if (_params._desc_size_bytes_wp % _params._aligment != 0)
        _desc_size_bytes_al++;
    _params._desc_size_bytes_wp = _desc_size_bytes_al * _params._aligment;

    int foffnbytes_alg = sizeof(uint64_t) / _params._aligment;
    if (sizeof(uint64_t) % _params._aligment != 0)
        foffnbytes_alg++;
    _params._feature_off_start = foffnbytes_alg * _params._aligment;
    _params._child_off_start =
        _params._feature_off_start +
        _params._m_k *
            _params._desc_size_bytes_wp; // where do children information start
                                         // from the start of the block

    // block: nvalid|f0 f1 .. fn|ni0 ni1 ..nin
    _params._block_size_bytes_wp =
        _params._feature_off_start +
        _params._m_k *
            (_params._desc_size_bytes_wp + sizeof(Vocabulary::block_node_info));
    _block_size_bytes_al = _params._block_size_bytes_wp / _params._aligment;
    if (_params._block_size_bytes_wp % _params._aligment != 0)
    ...
    _data = std::unique_ptr<char[], decltype(&AlignedFree)>(
        (char *)AlignedAlloc(_params._aligment, _params._total_size),
        &AlignedFree);

    memset(_data.get(), 0, _params._total_size);
}

void Vocabulary::transform(const cv::Mat &features, int level, fBow &result,
                           fBow2 &result2) {
    if (features.rows == 0)
        throw std::runtime_error("Vocabulary::transform No input data");
    if (features.type() != _params._desc_type)
        throw std::runtime_error("Vocabulary::transform features are of "
                                 "different type than vocabulary");
    if (features.cols * features.elemSize() != size_t(_params._desc_size))
        throw std::runtime_error(
            "Vocabulary::transform features are of different "
            "size than the vocabulary ones");
    ...
    // decide the version to employ according to the type of features, aligment
    ...
    if (_params._desc_type == CV_8UC1) {
        ...
        if (cpu_info->isSafeAVX() &&
            _params._aligment % 32 == 0) { // AVX version
            if (_params._desc_size == 256)
                _transform2<L2_avx_8w>(features, level, result,
                                       result2); // specific for surf 256 bytes
            else
                _transform2<L2_avx_generic>(features, level, result,
                                            result2); // any other
        }
        if (cpu_info->isSafeSSE() &&
            _params._aligment % 16 == 0) { // SSE version
            if (_params._desc_size == 256)
                _transform2<L2_sse3_16w>(
                    features, level, result,
                    result2); // specific for surf 256 bytes
            else
                _transform2<L2_se3_generic>(features, level, result,
                                            result2); // any other
        }
        // generic version
        _transform2<L2_generic>(features, level, result, result2);
    } else
        throw std::runtime_error("Vocabulary::transform invalid feature type. "
                                 "Should be CV_8UC1 or CV_32FC1");
}
...
fBow Vocabulary::transform(const cv::Mat &features) {
    ...
    if (_params._desc_type == CV_8UC1) {
        ...
        if (cpu_info->isSafeAVX() &&
            _params._aligment % 32 == 0) { // AVX version
            if (_params._desc_size == 256)
                result = _transform<L2_avx_8w>(
                    features); // specific for surf 256 bytes
            else
...
void Vocabulary::readFromFile(const std::string &filepath) {
    ...
    if (!file)
        throw std::runtime_error("Vocabulary::readFromFile could not open:" +
                                 filepath);
    fromStream(file);
}
...
void Vocabulary::fromStream(std::istream &str) {
    ...
    if (_data.get() == nullptr)
        throw std::runtime_error(
            "Vocabulary::fromStream Could not allocate data");
...
uint64_t fBow::hash() const {
    uint64_t seed = 0;
    for (auto e : *this)
        seed ^= e.first + int(e.second * 1000) + 0x9e3779b9 + (seed << 6) +
                (seed >> 2);
