...
void Vocabulary::transform(const cv::Mat &features, int level, fBow &result,
  ...
  if (features.type() != _params._desc_type)
    throw std::runtime_error(
        "Vocabulary::transform features are of different type than vocabulary");
  if (features.cols * features.elemSize() != size_t(_params._desc_size))
    throw std::runtime_error("Vocabulary::transform features are of different "
  ...
  if (_params._desc_type == CV_8UC1) {
    ...
    if (cpu_info->isSafeSSE() && _params._aligment % 16 == 0) { // SSE version
      if (_params._desc_size == 256)
        _transform2<L2_sse3_16w>(features, level, result,
                                 result2); // specific for surf 256 bytes
      else
...
fBow Vocabulary::transform(const cv::Mat &features) {
  ...
  if (_params._desc_type == CV_8UC1) {
    ...
    if (cpu_info->isSafeSSE() && _params._aligment % 16 == 0) { // SSE version
      if (_params._desc_size == 256)
        result =
            _transform<L2_sse3_16w>(features); // specific for surf 256 bytes
      else
...
uint64_t fBow::hash() const {
  ...
  for (auto e : *this)
    seed ^=
        e.first + int(e.second * 1000) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
