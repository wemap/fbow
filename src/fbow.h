...
class FBOW_API Vocabulary {
  static inline void *AlignedAlloc(int __alignment, int size) {
    ...
    *(ptr - 1) =
        (unsigned char)off; // save in prev, the offset  to properly remove it
    return ptr;
  }
...
private:
  void setParams(int aligment, int k, int desc_type, int desc_size, int nblocks,
  ...
  struct Block {
    Block(char *bsptr, uint64_t ds, uint64_t ds_wp, uint64_t fo, uint64_t co)
        : _blockstart(bsptr), _desc_size_bytes(ds), _desc_size_bytes_wp(ds_wp),
    ...
    template <typename T> inline T *getFeature(int i) {
      return (T *)(_blockstart + _feature_off_start + i * _desc_size_bytes_wp);
    }
    char *_blockstart;
  ...
  template <typename register_type, typename distType, int aligment> class Lx {
  public:
