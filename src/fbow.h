...
// Bag of words with augmented information. For each word, keeps information
// about the indices of the elements that have been classified into the word it
// is computed at the desired level
...
class FBOW_API Vocabulary {
    static inline void *AlignedAlloc(int __alignment, int size) {
        ...
        *(ptr - 1) = (unsigned char)
            off; // save in prev, the offset  to properly remove it
        return ptr;
    }
...
public:
    ...
    void transform(const cv::Mat &features, int level, fBow &result,
                   fBow2 &result2);
...
private:
    void setParams(int aligment, int k, int desc_type, int desc_size,
                   int nblocks, std::string desc_name);
    struct params {
        char _desc_name_[50]; // descriptor name. May be empty
        uint32_t _aligment = 0,
                 _nblocks = 0; // memory aligment and total number of blocks
        uint64_t _desc_size_bytes_wp =
            0; // size of the descriptor(includes padding)
        uint64_t _block_size_bytes_wp =
            0; // size of a block   (includes padding)
        uint64_t _feature_off_start =
            0; // within a block, where the features start
        uint64_t _child_off_start =
            0; // within a block,where the children offset part starts
        uint64_t _total_size = 0;
        int32_t _desc_type = 0,
                _desc_size =
                    0; // original descriptor types and sizes (without padding)
    ...
    struct block_node_info {
        ...
        // if leaf, returns the index of the feature it represents. In case of
        // bagofwords it must be a invalid value
        inline uint32_t getId() const {
            return (id_or_childblock & 0x7FFFFFFF);
        }

        // sets as leaf, and sets the index of the feature it represents and its
        // weight
        inline void setLeaf(uint32_t id, float Weight) {
            assert(!(id & 0x80000000)); // check msb is zero
            id_or_childblock = id;
            id_or_childblock |=
                0x80000000; // set the msb to one to distinguish from non leaf
    ...
    // a block represent all the child nodes of a parent, with its features and
    // also information about where the child of these are in the data structure
    // a block structure is as follow: N|isLeaf|BlockParentId|p|F0...FN|C0W0 ...
    // CNWN.. N :16 bits : number of nodes in this block. Must be <=branching
    // factor k. If N<k, then the block has empty spaces since block size is
    // fixed isLeaf:16 bit inicating if all nodes in this block are leaf or not
    // BlockParentId:31: id of the parent
    // p :possible offset so that Fi is aligned
    // Fi feature of the node i. it is aligned and padding added to the end so
    // that F(i+1) is also aligned CiWi are the so called block_node_info (see
    // structure up) Ci : either if the node is leaf (msb is set to 1) or not.
    // If not leaf, the remaining 31 bits is the block where its children are.
    // Else, it is the index of the feature that it represent Wi: float value
    // empkoyed to know the weight of a leaf node (employed in cases of
    // bagofwords)
    struct Block {
        Block(char *bsptr, uint64_t ds, uint64_t ds_wp, uint64_t fo,
              uint64_t co)
            : _blockstart(bsptr), _desc_size_bytes(ds),
              _desc_size_bytes_wp(ds_wp), _feature_off_start(fo),
              _child_off_start(co) {}
        Block(uint64_t ds, uint64_t ds_wp, uint64_t fo, uint64_t co)
            : _desc_size_bytes(ds), _desc_size_bytes_wp(ds_wp),
              _feature_off_start(fo), _child_off_start(co) {}
        ...
        inline void setLeaf(bool v) const {
            *((uint16_t *)(_blockstart) + 1) = 1;
        }

        inline void setParentId(uint32_t pid) {
            *(((uint32_t *)(_blockstart)) + 1) = pid;
        }
        inline uint32_t getParentId() {
            return *(((uint32_t *)(_blockstart)) + 1);
        }

        inline block_node_info *getBlockNodeInfo(int i) {
            return (block_node_info *)(_blockstart + _child_off_start +
                                       i * sizeof(block_node_info));
        }
        inline void setFeature(int i, const cv::Mat &feature) {
            memcpy(_blockstart + _feature_off_start + i * _desc_size_bytes_wp,
                   feature.ptr<char>(0), feature.elemSize1() * feature.cols);
        }
        inline void getFeature(int i, cv::Mat feature) {
            memcpy(feature.ptr<char>(0),
                   _blockstart + _feature_off_start + i * _desc_size_bytes,
                   _desc_size_bytes);
        }
        template <typename T> inline T *getFeature(int i) {
            return (T *)(_blockstart + _feature_off_start +
                         i * _desc_size_bytes_wp);
        }
        char *_blockstart;
        uint64_t _desc_size_bytes = 0; // size of the descriptor(without
                                       // padding)
        uint64_t _desc_size_bytes_wp =
            0; // size of the descriptor(includding padding)
        uint64_t _feature_off_start = 0;
        uint64_t _child_off_start =
            0; // into the block,where the children offset part starts
    };
    ...
    inline Block getBlock(uint32_t b) {
        ...
        return Block(_data.get() + b * _params._block_size_bytes_wp,
                     _params._desc_size, _params._desc_size_bytes_wp,
                     _params._feature_off_start, _params._child_off_start);
    }
    ...
    // information about the cpu so that mmx,sse or avx extensions can be
    // employed
    ...
    public:
        ...
        void setParams(int desc_size, int block_desc_size_bytes_wp) {
            ...
            _nwords = _block_desc_size_bytes_wp /
                      sizeof(register_type); // number of aligned words
            feature = static_cast<register_type *>(
                AlignedAlloc(aligment, _nwords * sizeof(register_type)));
            memset(feature, 0, _nwords * sizeof(register_type));
        }
        inline void startwithfeature(const register_type *feat_ptr) {
            memcpy(feature, feat_ptr, _desc_size);
        }
    ...
    struct L2_avx_generic : public Lx<uint64_t, float, 32> {
        inline float computeDist(uint64_t *ptr) {
    ...
    struct L1_32bytes : public Lx<uint64_t, uint64_t, 8> {
        inline uint64_t computeDist(uint64_t *feat_ptr) {
            return uint64_popcnt(feat_ptr[0] ^ feature[0]) +
                   uint64_popcnt(feat_ptr[1] ^ feature[1]) +
                   uint64_popcnt(feat_ptr[2] ^ feature[2]) +
                   uint64_popcnt(feat_ptr[3] ^ feature[3]);
        }
    ...
    struct L1_61bytes : public Lx<uint64_t, uint64_t, 8> {
        inline uint64_t computeDist(uint64_t *feat_ptr) {
            return uint64_popcnt(feat_ptr[0] ^ feature[0]) +
                   ...
                   uint64_popcnt(feat_ptr[2] ^ feature[2]) +
                   uint64_popcnt(feat_ptr[3] ^ feature[3]) +
                   uint64_popcnt(feat_ptr[4] ^ feature[4]) +
                   uint64_popcnt(feat_ptr[5] ^ feature[5]) +
                   uint64_popcnt(feat_ptr[6] ^ feature[6]) +
                   uint64_popcnt(feat_ptr[7] ^ feature[7]);
        }
        inline uint64_t uint64_popcnt(uint64_t n) {
            return std::bitset<64>(n).count();
        }
    ...
    template <typename Computer> fBow _transform(const cv::Mat &features) {
        Computer comp;
        ...
        fBow result;
        std::pair<DType, uint32_t> best_dist_idx(
            std::numeric_limits<uint32_t>::max(), 0); // minimum distance found
        ...
        for (int cur_feature = 0; cur_feature < features.rows; cur_feature++) {
            ...
            do {
                ...
                for (int cur_node = 0; cur_node < c_block.getN(); cur_node++) {
                    DType d =
                        comp.computeDist(c_block.getFeature<TData>(cur_node));
                    if (d < best_dist_idx.first)
                        best_dist_idx = std::make_pair(d, cur_node);
                }
                bn_info = c_block.getBlockNodeInfo(best_dist_idx.second);
                // if the node is leaf get word id and weight,else go to its
                // children
                if (bn_info->isleaf()) { // if the node is leaf get word id and
                                         // weight
    ...
    void _transform2(const cv::Mat &features, uint32_t storeLevel, fBow &r1,
                     fBow2 &r2) {
        Computer comp;
        comp.setParams(_params._desc_size, _params._desc_size_bytes_wp);
        using DType = typename Computer::DType; // distance type
